...
final class PayTokensCommand extends Command implements PluginOwned {

    private TokensAPI $plugin;

    public function __construct() {
        parent::__construct("paytokens");
        $this->setDescription("Pay tokens to another player");
        $this->setUsage("Usage: /paytokens <player> <amount>");
        $this->setPermission(Permission::PERM_PAYTOKENS);
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : bool{
        ...
        $this->plugin->removeTokens($sender->getName(), $amount);

        $this->plugin->addTokens($targetName, $amount);

        $sender->sendMessage((string) new Message("successfully-paid-tokens", ["{name}", "{amount}"], [$targetName, number_format($amount)]));
        return true;
    }
